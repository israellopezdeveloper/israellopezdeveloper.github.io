<script lang="ts">
  ...
  export let onSelect: (w: Work) => void = () => {};
...
>
  ...
  <div class="card__logoFrame">
    {#if w.thumbnail}
      ...
      />
    {:else}
      <div class="card__logoPlaceholder" aria-hidden="true"></div>
    {/if}
  </div>

  <div class="card__name" title={w.name}>{w.name}</div>
</article>
...
{#if open}
  ...
  >
    <div class="tooltip__title">{w.name}</div>

    {#if w.period_time}
      <div class="tooltip__meta">{formatPeriod(w)}</div>
    {/if}
